...
// app.use('/api/files', fileRoutes); // New line
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // ✅ FIX: Compare password with the hashed password from the database
    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return res.status(400).json({ error: 'Invalid email or password.' });
...
// Protected Routes
app.use('/api/users', verifyToken, userRoutes);
app.use('/api/files', verifyToken, fileRoutes); // ✅ FIX: Add the verifyToken middleware to the file route
...
app.post('/api/create-super-admin', async (req, res) => {
  try {
    ...
    // ✅ FIX: Hash the super admin's password
    const hashedPassword = await bcrypt.hash(password, 10);
    const newUser = new User({ fullname, email, password: hashedPassword, role: 'super-admin' });
    await newUser.save();
    res.status(201).json({ message: 'Super Admin created successfully!' });
