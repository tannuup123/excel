...
import { FaFacebook, FaTwitter, FaLinkedin, FaInstagram, FaSun, FaMoon, FaArrowRight } from 'react-icons/fa';
    ...
    <div className="bg-gray-900 dark:bg-gray-100 text-gray-100 dark:text-gray-900 font-sans leading-relaxed transition-colors duration-500">
      ...
      <header className="fixed top-0 left-0 w-full z-50 py-4 px-6 md:px-12 flex justify-between items-center bg-gray-800 dark:bg-white bg-opacity-90 dark:bg-opacity-90 backdrop-filter backdrop-blur-lg shadow-lg transition-colors duration-500">
        ...
        <nav className="hidden md:flex space-x-6">
          ...
          <Link to="/blog" className="text-gray-300 dark:text-gray-700 hover:text-white dark:hover:text-gray-900 transition-colors duration-300">Blog</Link>
        </nav>
      ...
      <section className="py-20 bg-gray-900 dark:bg-gray-100 transition-colors duration-500">
        <div className="container mx-auto px-6 max-w-5xl pt-16 text-center">
          <h1 className="text-5xl md:text-6xl font-extrabold text-white dark:text-gray-900 mb-4">Our Blog</h1>
      ...
      <section className="py-12 bg-gray-800 dark:bg-gray-200">
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          {/* Recent Posts Grid */}
          <h2 className="text-3xl font-bold text-white dark:text-gray-900 mb-8">Recent Articles</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recentPosts.map(post => (
              <Link to={post.link} key={post.id} className="block bg-gray-900 dark:bg-white rounded-2xl overflow-hidden shadow-md transition-transform hover:scale-[1.05] duration-300">
                <img src={post.image} alt={post.title} className="w-full h-48 object-cover" />
                <div className="p-6">
                  <p className="text-sm text-gray-400 dark:text-gray-600">{post.date}</p>
                  <h3 className="text-xl font-bold text-white dark:text-gray-900 mt-2 mb-4">{post.title}</h3>
                  <p className="text-gray-300 dark:text-gray-700 text-sm">{post.summary}</p>
